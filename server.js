...
app.post('/api/library/add', authenticate, (req, res) => {
    try {
        ...
        if (user.library.includes(beatId)) {
...
app.post('/api/beats', authenticate, upload.single('beat'), async (req, res) => {
    try {
        ...
        const { title, genre, bpm, description, tags, bpm2, swing, reverb, delay } = req.body;
        const beats = readData('beats.json');
        ...
        const newBeat = {
            ...
            bpm2: parseInt(bpm2) || null,
            swing: parseInt(swing) || 0,
            effects: { reverb: reverb === 'true', delay: delay === 'true' },
...
app.get('/api/beats', (req, res) => {
    try {
        ...
        const { sort, genre, search, limit = 20 } = req.query;
        
        if (search) {
            const term = search.toLowerCase();
            beats = beats.filter(b => 
                b.title.toLowerCase().includes(term) ||
                b.producerName.toLowerCase().includes(term) ||
...
app.post('/api/beats/:beatId/record', authenticate, upload.single('vocal'), async (req, res) => {
    try {
        ...
        const { title, description, tags, effects } = req.body;
        ...
        const newVersion = {
            ...
            description: description || '',
            tags: tags ? tags.split(',').map(t => t.trim()) : [],
            effects: effects ? JSON.parse(effects) : { reverb: false, delay: false, echo: false },
    ...
    try {
        ...
        if (type === 'beats' || !type) {
            const matchedBeats = beats.filter(beat =>
                ...
                beat.tags.some(t => t.toLowerCase().includes(q.toLowerCase()))
            ).map(beat => ({
                type: 'beat',
                id: beat.id,
                title: beat.title,
...
server.listen(PORT, () => {
    console.log(`\n🎵 8Trax Studio Server running on http://localhost:${PORT}`);
