...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        if (file.fieldname === 'beat') cb(null, path.join(__dirname, 'uploads/beats/'));
        else cb(null, path.join(__dirname, 'uploads/vocals/'));
    },
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        const newUser = {
            id: uuidv4(), username, email, displayName: displayName || username, bio: bio || '',
            password: hashedPassword, createdAt: new Date().toISOString(), points: 0,
            uploadedBeats: [], recordings: [], library: [], followers: [], following: [],
...
app.post('/api/comments/add', authenticate, (req, res) => {
    try {
        ...
        const user = users.find(u => u.id === req.user.id);
        const newComment = {
            id: uuidv4(), beatId, userId: req.user.id, username: user.username,
            displayName: user.displayName, avatar: user.avatar, comment,
...
app.post('/api/messages/send', authenticate, (req, res) => {
    try {
        ...
        const newMessage = {
            id: uuidv4(), from: req.user.id, fromName: fromUser.displayName, to, message,
...
app.post('/api/beats', authenticate, upload.single('beat'), async (req, res) => {
    try {
        ...
        const beats = readData('beats.json');
        const newBeat = {
            id: uuidv4(), title, genre: genre || 'Other', bpm: parseInt(bpm) || 120,
            description: description || '', tags: tags ? tags.split(',').map(t => t.trim()) : [],
            producerId: req.user.id, producerName: user.displayName || user.username,
            producerUsername: user.username, fileUrl: `/uploads/beats/${req.file.filename}`,
            filename: req.file.filename, createdAt: new Date().toISOString(),
            plays: 0, downloads: 0, versions: []
        };
        beats.push(newBeat);
...
app.get('/api/beats', (req, res) => {
    try {
        ...
        if (genre && genre !== 'all') beats = beats.filter(b => b.genre === genre);
        if (sort === 'popular') beats.sort((a, b) => (b.plays + b.downloads) - (a.plays + a.downloads));
        else beats.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
...
app.post('/api/beats/:beatId/record', authenticate, upload.single('vocal'), async (req, res) => {
    try {
        ...
        const newVersion = {
            id: uuidv4(), beatId: req.params.beatId, beatTitle: beat.title,
            vocalistId: req.user.id, vocalistName: user.displayName || user.username,
            vocalistUsername: user.username, title: title || `${user.displayName}'s version`,
            description: description || '', tags: tags ? tags.split(',').map(t => t.trim()) : [],
            fileUrl: `/uploads/vocals/${req.file.filename}`, filename: req.file.filename,
            createdAt: new Date().toISOString(), votes: [], rating: 0, plays: 0
        };
...
app.get('/api/feed', authenticate, (req, res) => {
    try {
        ...
        const feedItems = [];
        beats.forEach(beat => {
            if (followingIds.includes(beat.producerId)) feedItems.push({ type: 'beat', ...beat, timestamp: beat.createdAt });
            beat.versions.forEach(version => {
                if (followingIds.includes(version.vocalistId)) feedItems.push({ type: 'recording', beat: { id: beat.id, title: beat.title }, ...version, timestamp: version.createdAt });
            });
        });
...
app.get('/api/trending', (req, res) => {
    try {
        ...
        beats.forEach(beat => {
            ...
            if (popularity > 5) trending.push({ type: 'beat', ...beat, popularity });
            beat.versions.forEach(version => {
...
app.get('/api/leaderboard', (req, res) => {
    try {
        ...
        const topUsers = users.sort((a, b) => (b.points || 0) - (a.points || 0)).slice(0, 20).map(user => ({
            ...
            points: user.points || 0, uploadedBeats: user.uploadedBeats?.length || 0,
            recordings: user.recordings?.length || 0, followers: user.followers?.length || 0, bio: user.bio || ''
        }));
        res.json(topUsers);
    } catch (error) { res.status(500).json({ error: error.message }); }
});
...
app.get('/api/search', (req, res) => {
    try {
        ...
        let results = [];
        if (type === 'users' || !type) {
            const matchedUsers = users.filter(user => user.username.toLowerCase().includes(q.toLowerCase()) || user.displayName?.toLowerCase().includes(q.toLowerCase()))
        ...
        if (type === 'beats' || !type) {
            const matchedBeats = beats.filter(beat => beat.title.toLowerCase().includes(q.toLowerCase()) || beat.producerName.toLowerCase().includes(q.toLowerCase()))
...
app.get('/api/users/search', (req, res) => {
    try {
        ...
        if (!q || q.length < 2) return res.json([]);
        const users = readData('users.json');
        const results = users.filter(user => user.username.toLowerCase().includes(q.toLowerCase()) || user.displayName?.toLowerCase().includes(q.toLowerCase()))
            .map(user => ({ id: user.id, username: user.username, displayName: user.displayName, avatar: user.avatar, points: user.points }))
            .slice(0, 20);
        res.json(results);
    } catch (error) { res.status(500).json({ error: error.message }); }
});
...
app.get('/api/users/:username', (req, res) => {
    try {
        ...
        const { password, ...userWithoutPassword } = user;
        res.json({
            ...userWithoutPassword, uploadedBeatsCount: userBeats.length, recordingsCount: userRecordings.length,
            uploadedBeats: userBeats, recordings: userRecordings
        });
    } catch (error) { res.status(500).json({ error: error.message }); }
});
